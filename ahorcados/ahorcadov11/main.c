...
int main()
{
    ...
    int dificultad,cant_jugadores,volver=1,longitud, letras_acertadas=0, vidas=6, aux, diml_equivocadas=-1, fin=0, diml_dichas=-1;//diml_equivocadas=-1 no significa ninguna posicion, es para decir que está vacío
    ...
    if(cant_jugadores==1){
        ...
        scanf("%d", &dificultad);
        puntero_palabra=palabra_random(dificultad);//recibe un puntero a una palabra
...
char* palabra_random(int dificultad){
    int x;
